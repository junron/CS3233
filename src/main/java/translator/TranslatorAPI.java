...
//    Set fake user agent so we dont get blobked
    connection
            .setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36");
    connection.setDoOutput(true);

    int status = connection.getResponseCode();

    BufferedReader readStream = new BufferedReader(
            status > 299 ? new InputStreamReader(connection.getErrorStream(), StandardCharsets.UTF_8)
...
//    Construct full url for using google translate
    ...
    String params = "client=gtx&sl=" + sourceLang + "&tl=" + targetLang + "&dt=t&q=" +
